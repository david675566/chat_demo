...
class ChatRoomView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider<ChatBloc>(
      create: (context) {
        return ChatBloc(chatRepository: chatRepo)
          ..add(RequestGetMessages(conversationId: conversationId));
      },
      child: BlocBuilder<ChatBloc, ChatState>(
        buildWhen: (previous, current) => previous != current,
        builder: (context, state) {
          final messages =
              (state is ChatReady) ? state.data : <chat_types.Message>[];
          return Scaffold(
            appBar: AppBar(
              title: Text(participants.map((e) => e.firstName!).join('&')),
            ),
            body: Column(
              children: [
                Expanded(
                  child: ListView.separated(
                    ...
                    separatorBuilder:
                        (context, index) => const SizedBox(height: 9),
                    itemCount: messages.length,
                    itemBuilder: (context, index) {
                      final msg = messages[index];
                      final isOutgoing =
                          (chatRepo.currentUser.id == msg.author.id);

                      return MessageWidget(
                        message: messages[index],
                        isOutgoing: isOutgoing,
                        reactions:
                            ChatRepository.emojiToStringMap.keys.toList(),
                        useNonBubbleSystemMessage:
                            true, // Change this if you want to use the non-bubble system message
